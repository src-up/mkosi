...
def run_qemu(args: Args, config: Config) -> None:
    ...
    with contextlib.ExitStack() as stack:
        ...
        if kernel and (kerneltype != KernelType.uki or not config.architecture.supports_smbios(firmware)):
            ...
            # Obstacle: for case when Bootloader=uki-signed, the UKI is already built by the distro so
            ...
            # uki-signed so it gets passed to systemd-stub via this mechanism.
            # We can safely assume stub_kcl will be invoked for all uki-signed use
            # cases because: (1) uki-signed always boots via UEFI so supports_smbios()
            # is always true for it, and (2) systemd-stub is guaranteed to be present
            # in the uki-signed UKI since .sdmagic — a PE section that systemd-stub
            # adds when wrapping a kernel into a UKI — is required by mkosi to qualify
            # the .efi as a UKI and therefore be copied into the ESP as a uki-signed image.
            stub_kcl = (config.kernel_command_line if config.bootloader == Bootloader.uki_signed else []) + kcl
            cmdline += [
                "-smbios",
